...
export function RevenueForecastWidget() {
  const { data, loading, error, isAvailable, isLocked } = useRevenueForecast()
  const { currencyCode } = useCurrency()

  // Locked state (Enterprise-only feature)
  if (isLocked || (!isAvailable && !loading && !error)) {
    return (
  ...
  // Safety check: data should exist at this point, but be defensive
  if (!data?.period_30_days || !data?.period_60_days || !data?.period_90_days) {
    return (
      <Card className="border-gray-700/50">
        <CardHeader>
          <h3 className="text-lg font-bold text-white">📊 Revenue Forecast</h3>
        ...
        <CardBody>
          <div className="text-gray-400 text-sm">Unable to load forecast data</div>
        </CardBody>
      </Card>
    )
  }
    ...
    <Card className="border-blue-500/30">
      ...
      <CardBody>
        <div className="space-y-4 md:space-y-6">
          ...
          {/* Notes */}
          {data.period_30_days?.notes && data.period_30_days.notes.length > 0 && (
