        ...
        <header className="backdrop-blur-xl border-b border-white/10 sticky top-0 z-20">
          <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex justify-between items-center gap-2 md:gap-0">
            <Link href="/" className="flex items-center gap-3">
              <img src="/logo.svg" alt="Verkspay Logo" className="h-12 md:h-14 w-auto" />
              <span className="text-2xl md:text-3xl font-bold text-blue-400">Verkspay</span>
            </Link>
              ...
              >
                ...
                <ul className="space-y-2 mb-6 flex-1">
                  {plan.features.map((feature, j) => {
                    ...
                    return (
                      <li key={j} className={`flex items-start gap-2 text-sm ${isBold ? 'mt-2' : ''}`}>
                        {!isBold && <span className="text-blue-400 mt-1 flex-shrink-0">✓</span>}
                        <span className={isBold ? 'font-semibold text-gray-100' : 'text-gray-300'}>
                          {displayText}
                        </span>
                      </li>
                    )
