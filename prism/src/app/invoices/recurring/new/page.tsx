...
export default function NewRecurringInvoice() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (profileData?.currency_code) {
        setFormData((prev) => ({ ...prev, currency_code: profileData.currency_code }))
      }
    }
  ...
  const handleSave = async (status: 'active' | 'paused') => {
    ...
    try {
      // Calculate next_generate_date (start today if start_date is today or past)
      const startDate = new Date(formData.start_date)
      const today = new Date()
      const nextDate = startDate <= today ? today : startDate
        ...
        .insert([
          {
            ...
            status: status,
            next_generate_date: nextDate.toISOString().split('T')[0],
            invoices_generated: 0
          }
        ])

      if (error) {
        console.error('[NewRecurringInvoice] Error:', error)
        setMessage('❌ Failed to create recurring invoice')
        setSaving(false)
        return
      }

      setMessage(`✅ Recurring invoice created and ${status === 'active' ? 'activated' : 'saved as paused'}!`)
      setTimeout(() => router.push('/invoices/recurring'), 1500)
    } catch (err) {
      console.error('[NewRecurringInvoice] Error:', err)
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-8">
        <h1 className="text-4xl font-bold text-white mb-10">Create Recurring Invoice</h1>
