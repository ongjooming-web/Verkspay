...
function getPlanFromPriceId(priceId: string): string | undefined {
  const priceMap: { [key: string]: string } = {
    [process.env.PRISM_STARTER_MONTHLY || '']: 'starter',
    [process.env.PRISM_STARTER_ANNUAL || '']: 'starter',
    [process.env.PRISM_PRO_MONTHLY || '']: 'pro',
    [process.env.PRISM_PRO_ANNUAL || '']: 'pro',
    [process.env.PRISM_ENTERPRISE_MONTHLY || '']: 'enterprise',
    [process.env.PRISM_ENTERPRISE_ANNUAL || '']: 'enterprise',
  }
      ...
      case 'customer.subscription.updated': {
        ...
        if (!planToUpdate) {
          ...
          console.error('[Webhook] Available env vars:', {
            starter_monthly: !!process.env.PRISM_STARTER_MONTHLY,
            starter_annual: !!process.env.PRISM_STARTER_ANNUAL,
            pro_monthly: !!process.env.PRISM_PRO_MONTHLY,
            pro_annual: !!process.env.PRISM_PRO_ANNUAL,
            enterprise_monthly: !!process.env.PRISM_ENTERPRISE_MONTHLY,
            enterprise_annual: !!process.env.PRISM_ENTERPRISE_ANNUAL,
          })
          break
        }
